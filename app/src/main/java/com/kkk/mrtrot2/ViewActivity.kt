...
//        rewardedAd = RewardedAd(this, "ca-app-pub-8246055051187544/3073413640")
        ...
        val adLoadCallback = object : RewardedAdLoadCallback() {
            override fun onRewardedAdLoaded() {
                // Ad successfully loaded. 광고를 성공적으로 받아옴
...
//        bigImage.setImageResource(img)
        ...
        shareButton.setOnClickListener { try {
            val sendText = "감동의 무대를 선사한 국민가수 함께 응원해요! 다운로드 - https://play.google.com/store/apps/details?id=com.kkk.tosinger"
    ...
    private fun admobSetting() {
        ...
        if (rewardedAd.isLoaded) {
            val activityContext: Activity = this@ViewActivity
            val adCallback = object : RewardedAdCallback() {
                ...
                override fun onUserEarnedReward(@NonNull reward: RewardItem) {
                    // User earned reward.
                    Log.d("TAG", "리워드 받았다!!!" + reward.amount)
                    val pref = getSharedPreferences("pref", 0)
            ...
            val adLoadCallback = object : RewardedAdLoadCallback() {
                override fun onRewardedAdLoaded() {
                    // Ad successfully loaded. 광고를 성공적으로 받아옴
                    Log.d("애드몹리워드로드성공?","리워드 로드 성공")
                }

                override fun onRewardedAdFailedToLoad(adError: LoadAdError) {
                    // Ad failed to load. 광고를 못 받아옴 나중에 토스트메세지
                    Log.d("애드몹리워드로드실패?","리워드 로드 실패")

                }
            }
            rewardedAd.loadAd(AdRequest.Builder().build(), adLoadCallback)
            Log.d("애드몹리워드쇼","애드몹 쇼 되나?")
